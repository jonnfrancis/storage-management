...
export const createAccount = async ({ fullName, email }: { fullName: string; email: string;}) => {
    ...
    if(!existingUser) {
        ...
        await databases.createDocument( appwriteConfig.databaseId, appwriteConfig.usersCollectionId, ID.unique(), {fullName, email, avatar: avatarPlaceholderUrl, accountId });
    }
...
export const signInUser = async ({ email }: { email: string }) => {
    try {
        ...
        return parseStringify({accountId: null, error: "User not found. Please create an account."});
    } catch (error) {
